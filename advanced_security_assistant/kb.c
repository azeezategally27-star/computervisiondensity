// kb.c - SQLite FTS5 KB for security assistant
#include "kb.h"
#include <sqlite3.h>
#include <stdio.h>
#include <stdlib.h>
...
int kb_init(const char *dbpath){
    ...
    const char *sql = "CREATE VIRTUAL TABLE IF NOT EXISTS secdocs USING fts5(filename, content);";
    char *err = NULL;
...
int kb_train_from_dir(const char *dir_path){
    ...
    sqlite3_stmt *stmt = NULL; const char *ins = "INSERT INTO secdocs(filename, content) VALUES(?, ?);";
    while((ent = readdir(d))){
...
char **kb_query(const char *query, int max_results, int *out_count){
    char sql[4096]; snprintf(sql, sizeof(sql), "SELECT filename || '::' || snippet(secdocs, 1, '[', ']', '...', -1) AS s FROM secdocs WHERE secdocs MATCH ? LIMIT %d;", max_results);
